...
router.get("/jobs", async (req, res) => {
    ...
    const conditions = [];
    if (query.category) {
        conditions.push(eq(jobsTable.category, query.category));
    }
    if (query.search) {
        const term = `%${query.search}%`;
        conditions.push(or(like(jobsTable.title, term), like(jobsTable.company, term), like(jobsTable.location, term), like(jobsTable.description, term), like(jobsTable.eligibility, term), like(jobsTable.salary, term)));
    }
