...
import { UpdateApplicationStatusBody, UpdateApplicationStatusParams, GetJobApplicantCountParams, internalApplicationSchema, } from "@workspace/api-zod";
    ...
    try {
        ...
        if (existing) {
            ...
            if (existing.status !== "Redirected") {
                [app] = await db.update(applicationsTable).set({ status: "Redirected" }).where(eq(applicationsTable.id, existing.id)).returning();
            }
            else {
                app = existing;
            }
...
router.get("/applications", async (req, res) => {
    ...
    if (jobId) {
        ...
        baseQuery = baseQuery.where(sql `${applicationsTable.userId} = ${user.id} OR ${applicationsTable.applicantEmail} = ${user.email}`);
    }
...
router.post("/applications", async (req, res) => {
    ...
    const [existing] = await db
        .select()
        .from(applicationsTable)
        .where(and(jobId ? eq(applicationsTable.jobId, jobId) : eq(applicationsTable.examId, examId), eq(applicationsTable.applicantEmail, user.email || applicantEmail)));
    if (existing) {
    ...
    try {
        ...
        if (user || body.applicantEmail) {
            const [existing] = await db
                .select()
                .from(applicationsTable)
                .where(and(eq(applicationsTable.jobId, parseInt(body.jobId)), eq(applicationsTable.applicantEmail, user?.email || body.applicantEmail)));
            if (existing) {
